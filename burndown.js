        ...
        for (let i = 0; i <= (100 - startAge); i++) {
            ...
            if (!isRet) {
                ...
                if (persona === 'PLATINUM') {
                    ...
                    // 401k = 1.0. Metals = GainRatio/0.72 (Penalty weight). Brokerage = GainRatio. Cash = 0.
                    let assets = [
                    ...
                    assets.forEach(a => {
                        if (a.type === 'Collectibles' && a.val > 0) {
                            const gainRatio = 1 - (a.basis / a.val);
                            a.density = gainRatio / 0.72; // Weighting for 28% potential tax
                        } else if (a.type === 'LTCG' && a.val > 0) {
                            a.density = 1 - (a.basis / a.val);
                        }
                    });
                    ...
                    // PASS 1: MAGI FILL (Sort by Density Descending)
                    ...
                    for (let asset of magiFillAssets) {
                        ...
                        // Draw = Room / Density (Inverse logic: How much draw to create X taxable income?)
                        // If density is > 0.
                        let drawNeeded = room / asset.density;
                        let draw = Math.min(drawNeeded, asset.val);
                        
                        bestDraws[asset.key] = (bestDraws[asset.key] || 0) + draw;
                        pass1CashGenerated += draw;
                        
                        let taxablePortion = draw * asset.density; // Approx
                        if (asset.type === 'Ordinary') currentOrdDraw += draw;
                        else if (asset.type === 'Collectibles') currentCollDraw += (draw * (1 - (asset.basis/asset.val)));
                        else if (asset.type === 'LTCG') currentLtcgDraw += (draw * (1 - (asset.basis/asset.val)));
                        
                        // Update tracking for next asset in loop
                        if (asset.key === 'metals') {
                             // Re-calc specific gain for tracking accurate room
                             const gain = draw * (1 - (asset.basis / asset.val));
                             currentMagi += gain;
                        } else if (asset.type === 'LTCG') {
                             const gain = draw * (1 - (asset.basis / asset.val));
                             currentMagi += gain;
                        } else {
                             currentMagi += draw;
                    ...
                    let currentTotalCash = floorGross + pass1CashGenerated + snap - taxes;
                    ...
                    if (gap > 0) {
                        ...
                        let zeroAssets = assets.filter(a => a.density === 0 && a.key !== 'heloc').concat(assets.filter(a => a.key === 'heloc')); // HELOC last
